...
class ContentSource(Base):
    ...
    status: Mapped[str] = mapped_column(
        Enum("draft", "approved", "archived", name="content_status"),
        default="draft",
        nullable=False,
    )
...
class ContentSection(Base):
    ...
    parent_section_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("content_sections.id"), nullable=True
    )
    ...
    source: Mapped["ContentSource"] = relationship(
        "ContentSource", back_populates="sections", foreign_keys=[source_id]
    )
    children: Mapped[list["ContentSection"]] = relationship(
        "ContentSection", foreign_keys=[parent_section_id], lazy="selectin"
    )
    reference: Mapped["ContentReference | None"] = relationship(
        "ContentReference", back_populates="section", uselist=False
    )
...
class ContentReference(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    source_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("content_sources.id"), nullable=False
    )
    section_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("content_sections.id"), nullable=False
    )
