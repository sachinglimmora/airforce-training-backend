...
class Checklist(Base):
    ...
    items: Mapped[list["ChecklistItem"]] = relationship("ChecklistItem", back_populates="checklist", order_by="ChecklistItem.ordinal")
...
class ChecklistItem(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    checklist_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("checklists.id"), nullable=False)
    ...
    mode: Mapped[str] = mapped_column(
        Enum("challenge_response", "read_do", "do_verify", name="checklist_mode"), nullable=False, default="challenge_response"
    )
...
class ChecklistSession(Base):
    ...
    status: Mapped[str] = mapped_column(
        Enum("in_progress", "completed", "aborted", name="checklist_session_status"), default="in_progress"
    )
