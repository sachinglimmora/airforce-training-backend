...
class TrainingSession(Base):
    ...
    session_type: Mapped[str] = mapped_column(
        Enum("theory", "checklist", "procedure", "scenario", "vr", "assessment", name="training_session_type"),
        nullable=False,
    )
    ...
    status: Mapped[str] = mapped_column(
        Enum("in_progress", "completed", "aborted", name="training_session_status"), default="in_progress"
    )
...
class SessionEvent(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    session_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("training_sessions.id"), nullable=False, index=True)
    event_type: Mapped[str] = mapped_column(String(64), nullable=False)
    step_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("procedure_steps.id"), nullable=True)
