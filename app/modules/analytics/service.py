...
class AnalyticsService:
    ...
    async def get_compliance_report(self) -> dict:
        ...
        q = select(
            ...
            func.count(Deviation.id).label("count")
        ...
        failed_sessions_q = select(func.count(func.distinct(Deviation.session_id))).where(Deviation.severity == 'critical')
        failed_sessions = (await self.db.execute(failed_sessions_q)).scalar() or 0
        
        pass_rate = ((total_sessions - failed_sessions) / total_sessions * 100) if total_sessions > 0 else 100
