...
class ProcedureService:
    ...
    async def complete_step(
        self, session_id: str, step_id: str, trainee_id: str, elapsed_ms: Optional[int] = None, notes: Optional[str] = None
    ) -> dict:
        # 1. Validate session and step
        result = await self.db.execute(select(ProcedureSession).where(ProcedureSession.id == session_id))
        ...
        if step.target_time_seconds and elapsed_ms and (elapsed_ms / 1000) > step.target_time_seconds:
             deviation = Deviation(
                session_id=session.id,
                step_id=step.id,
