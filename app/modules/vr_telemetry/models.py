...
class VRSession(Base):
    ...
    training_session_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("training_sessions.id"), nullable=False)
    device_id: Mapped[str] = mapped_column(String(128), nullable=False)
    device_type: Mapped[str] = mapped_column(String(64), nullable=False)
    runtime: Mapped[str] = mapped_column(Enum("webxr", "unity", name="vr_runtime"), nullable=False, default="webxr")
...
class VRTelemetryEvent(Base):
    ...
    vr_session_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("vr_sessions.id"), nullable=False, index=True)
