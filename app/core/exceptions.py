...
class TokenInvalid(AegisException):
    def __init__(self):
        super().__init__(
            status.HTTP_401_UNAUTHORIZED, "TOKEN_INVALID", "Token signature or structure is invalid"
        )
...
class PIIDetected(AegisException):
    def __init__(self):
        super().__init__(
            status.HTTP_403_FORBIDDEN, "PII_DETECTED", "Request blocked: personal data detected"
        )
...
class AccountLocked(AegisException):
    def __init__(self):
        super().__init__(
            423, "ACCOUNT_LOCKED", "Account locked due to too many failed login attempts"
        )
...
class TooManyAttempts(AegisException):
    def __init__(self):
        super().__init__(
            status.HTTP_429_TOO_MANY_REQUESTS, "TOO_MANY_ATTEMPTS", "Too many login attempts"
        )
...
class CitationNotFound(AegisException):
    def __init__(self, key: str):
        super().__init__(
            status.HTTP_400_BAD_REQUEST, "CITATION_NOT_FOUND", f"Citation key not found: {key}"
        )
...
class ValidationFailed(AegisException):
    def __init__(self, details: list):
        super().__init__(
            status.HTTP_400_BAD_REQUEST, "VALIDATION_FAILED", "Request validation failed", details
        )
