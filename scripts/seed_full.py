...
PROCEDURES = [
    (
        ...
        [
            ...
            (
                5,
                "ENG START switch — OFF when N1 stable",
                "ENGINE STABLE",
                "do_verify",
                20,
                True,
            ...
            (
                9,
                "ENG START switch ENG 2 — OFF when stable",
                "ENGINE STABLE",
                "do_verify",
                20,
                True,
    ...
    (
        ...
        [
            ...
            (
                ...
                True,
                6,
                "fire persists after bottle 1",
            ),
            (
                9,
                "Declare MAYDAY — divert to nearest field",
                None,
                "do_verify",
                10,
                True,
...
COMPETENCIES = [
    (
        ...
        "Technical",
        "Follows SOPs and checklists without deviation",
    ),
    ...
    (
        ...
        "Decision Making",
        "Cognitive",
        "Makes timely and correct decisions under pressure",
    ),
    (
        "CRM",
        "Crew Resource Management",
        "Non-Technical",
        "Effectively coordinates with crew and ATC",
    ),
    (
        "SIT-AWR",
        "Situational Awareness",
        "Cognitive",
        "Maintains awareness of aircraft state and environment",
    ),
    (
        "WORKLOAD",
        "Workload Management",
        "Non-Technical",
        "Prioritises tasks and manages workload under stress",
    ),
]
...
async def _get_or_create(
    db: AsyncSession, model, filter_kwargs: dict, create_kwargs: dict | None = None
):
    stmt = select(model)
        ...
        async with db.begin():
            ...
            for name, desc in ROLES:
                r, created = await _get_or_create(
                    db, Role, {"name": name}, {"name": name, "description": desc}
                )
                role_map[name] = r
                if created:
                    print(f"  + role: {name}")
            ...
            for cl_name, phase, ac_code, items in CHECKLISTS:
                cl, created = await _get_or_create(
                    ...
                    Checklist,
                    {"name": cl_name},
                    {"name": cl_name, "phase": phase, "aircraft_id": aircraft_map[ac_code].id},
                )
                if created:
            ...
            for sc in SCENARIOS:
                ...
                _, created = await _get_or_create(
                    ...
                    {"scenario_code": sc["scenario_code"]},
                    {
                        ...
                        if sc["scenario_type"] == "engine_fire"
                        else None,
                    },
                )
                if created:
