...
async def seed_sessions():
    ...
    async with AsyncSession_() as db:
        async with db.begin():
            ...
            trainee1 = (
                await db.execute(select(User).where(User.email == "trainee1@aegis.internal"))
            ).scalar_one_or_none()
            instructor = (
                await db.execute(select(User).where(User.email == "instructor@aegis.internal"))
            ).scalar_one_or_none()
            evaluator = (
                await db.execute(select(User).where(User.email == "evaluator@aegis.internal"))
            ).scalar_one_or_none()
            ...
            procedure = (
                await db.execute(
                    select(Procedure).where(Procedure.name == "Engine Fire — In Flight")
                )
            ).scalar_one_or_none()
            scenario = (
                await db.execute(
                    select(Scenario).where(Scenario.scenario_code == "ENG-FIRE-INFLIGHT-B737")
                )
            ).scalar_one_or_none()
            rubric = (
                await db.execute(
                    select(Rubric).where(Rubric.name == "Standard Procedure Evaluation")
                )
            ).scalar_one_or_none()
            competency_sys = (
                await db.execute(select(Competency).where(Competency.code == "SYS-KNW"))
            ).scalar_one_or_none()
            competency_dec = (
                await db.execute(select(Competency).where(Competency.code == "DECISION"))
            ).scalar_one_or_none()
