...
async def seed_synthetic_fcom(db: AsyncSession, aircraft_id: uuid.UUID | None = None) -> ContentSource:
    ...
    Each call uses a fresh UUID-prefixed citation_key so tests are isolated even
    when conftest's session.rollback() doesn't undo a committed seed (e.g. on
    Sachin's main-branch conftest which yields a session but doesn't truncate
    between tests).
    """
    # Unique-per-call prefix for citation keys.
    suffix = uuid.uuid4().hex[:8]
    source = ContentSource(
        id=uuid.uuid4(),
        source_type="fcom",
        aircraft_id=aircraft_id,
        title=f"Synthetic FCOM {suffix}",
        version="Rev 1",
